def deep_clone(haaash)
  Marshal.load(Marshal.dump(haaash))
end

def label(*directions)
  directions.join(",")
end
...
def adjacent_values(tiles, coordinates)
  coordinates.map { |(xx, yy, zz)| tiles[label(xx, yy, zz)] || "W" }
end
